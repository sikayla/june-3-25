...
// This page should only be accessible to logged-in venue owners
...
$reservation = null;
try {
    // Fetch the reservation details, ensuring it belongs to a venue owned by the logged-in user
    // Corrected: Joined with client_info to get venue contact details
    // Corrected: Selected v.location instead of v.address
    $sql = "SELECT
                ...
                v.title AS venue_name, v.location AS venue_location, -- Using v.location as the venue's address
            ...
            WHERE r.id = :reservation_id AND v.user_id = :owner_user_id"; // Ensure owner owns the venue
    ...
    if (!$reservation) {
        die("Error: Reservation not found or you do not have permission to view this receipt.");
    }
